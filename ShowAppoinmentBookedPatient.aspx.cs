...
public partial class ShowAppoinmentBookedPatient : System.Web.UI.Page
{
    ...
    protected void GVTestentry_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowIndex == -1)
            return;
       bool Mailstatus = Convert.ToBoolean(e.Row.Cells[09].Text);
       if (Mailstatus == false)
       {
           e.Row.Cells[09].Text = "<span class='btn btn-xs btn-danger' >NA</span>";

       }
       else
       {
           e.Row.Cells[09].Text = "<span class='btn btn-xs btn-success' >Done</span>";
       }
       int DirPAt = Convert.ToInt32(e.Row.Cells[10].Text);
       if (DirPAt == 0)
       {
       ...
       {
           e.Row.Cells[10].Text = "<span class='btn btn-xs btn-success' >Direct</span>";
       }
    ...
    protected void btnreport_Click(object sender, EventArgs e)
    {
        ...
        string query = "ALTER VIEW [dbo].[VW_GetAppoinmentBook] AS (select Initial+' '+PatientFirstName as PatientName,CONVERT(varchar(15),CAST(AppTime AS TIME),100) as AppoinmentTime, case when DirectApp=0 then 'NA' else 'Direct App' end as DirectAppoint ,* from tbl_Appointment ";
        //" ";
        if (fromdate.Text != "" && todate.Text != "")
        {
            query += " where (CAST(CAST(YEAR(AppDate) AS varchar(4)) + '/' + CAST(MONTH(AppDate) AS varchar(2)) + '/' + CAST(DAY(AppDate) AS varchar(2))  AS datetime) between ('" + Convert.ToDateTime(fromdate.Text).ToString("MM/dd/yyyy") + "') and ('" + Convert.ToDateTime(todate.Text).ToString("MM/dd/yyyy") + "') )";
        }
        if (Session["usertype"].ToString().Trim() == "CollectionCenter")
        {
            createuserTable_Bal_C ui = new createuserTable_Bal_C(Session["username"].ToString(), Convert.ToInt32(Session["Branchid"]));

            query = query + " and CenterCode = '" + ui.CenterCode.Trim() + "' ";
        }
        ...
        Session.Add("rptsql", "");
        Session["rptname"] = Server.MapPath("~/DiagnosticReport/Rpt_ShowAppoinmentPatient.rpt");
        Session["reportname"] = "Rpt_ShowAppoinmentPatient";
