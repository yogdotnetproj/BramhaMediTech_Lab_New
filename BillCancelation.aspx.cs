...
public partial class BillCancelation : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            try
            {
                if (Convert.ToString(Session["HMS"]) != "Yes")
                {
                    ...
                    {
                        checkexistpageright("BillCancelation.aspx");
                    }
                }
    ...
    public static string[] Getcenter(string prefixText, int count)
    {
        ...
        if (labcode != null && labcode != "")
        {
            sda = new SqlDataAdapter("SELECT * FROM DrMT where DoctorName like '" + prefixText + "%' and DrCheck_flag='CC' and LBcode='" + labcode.ToString().Trim() + "' and branchid=" + branchid + " order by DoctorName", con);
        }
        else
        {
            sda = new SqlDataAdapter("SELECT * FROM DrMT where DoctorName like '" + prefixText + "%' and DrCheck_flag='CC' and branchid=" + branchid + " order by DoctorName", con);
        }
