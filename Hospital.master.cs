...
public partial class SiteMaster : System.Web.UI.MasterPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
           ...
           // path = patho[0].ToString().Split('/');
            ...
            UsernameLB2.Text = Convert.ToString(Session["username"]);
            ...
            if (Convert.ToString(Session["HMS"]) == "Yes")
            {
                ...
                // Session["username"] = Convert.ToString(Request.QueryString["username"]);
                // Session["usertype"] = Convert.ToString(Request.QueryString["UserType"]);
                LoadSlideBar_HMS(Username, Convert.ToString(Session["ModuleId"]));
            }
            else
            {
    ...
    private void LoadSlideBar_HMS(string UserNm, string MainModule)
    {
        string SlideBarHtml = "";
        ...
        string MenuSQL = "";
        string LabName = Convert.ToString(Session["LabName"]);
        if ((Convert.ToString(Session["usertype"]).Trim() == "Administrator") && MainModule == "0")
        {
            MenuSQL = String.Format(@"select MenuID , MenuName,icon from TBL_MenuMaster where Isactive=1 and ModuelName='" + LabName + "' ");
        }
        else if ((Convert.ToString(Session["usertype"]).Trim() == "Administrator") && MainModule != "0")
        {
            MenuSQL = String.Format(@"select MenuID , MenuName,icon from TBL_MenuMaster where Isactive=1  and ModuelName='" + LabName + "'");// and Moduleid = '" + MainModule + "
        }
        else if (Convert.ToString(Session["usertype"]).Trim() == "Administrator")
        {
            MenuSQL = String.Format(@"select MenuID , MenuName,icon from TBL_MenuMaster where Isactive=1  and ModuelName='" + LabName + "'");// and Moduleid = '" + MainModule + "
        }
        ...
        else
        {
            MenuSQL = String.Format(@"SELECT DISTINCT    " +
                     ...
                     "  WHERE  ModuelName='" + LabName + "' and    (CTuser.USERNAME = '" + Convert.ToString(Session["username"]) + "') and  TBL_SubMenuMaster.Isvisible=1  " +
                     "  order by TBL_MenuMaster.MenuID   ");
        }
        string connectionString = ConfigurationManager.ConnectionStrings["DBHospitalI"].ConnectionString;
        SqlConnection con = new SqlConnection(connectionString);
        ...
        foreach (DataRow MainDr in MenuDt.Rows)
        {
            ...
            if (Convert.ToString(Session["usertype"]).Trim() == "Administrator" || Convert.ToString(Session["usertype"]).Trim() == "Administrator")
            {

                SubMenuSQL = String.Format(@"select SubMenuID as MenuID,SubMenuName ,SubMenuNavigateURL ,TBL_MenuMaster.ModuelName from TBL_SubMenuMaster INNER JOIN TBL_MenuMaster ON TBL_SubMenuMaster.MenuID = TBL_MenuMaster.MenuID where TBL_MenuMaster.MenuID='" + Menuid + "'  and  TBL_SubMenuMaster.Isvisible=1 and ModuelName='" + LabName + "'", con);
            }
            else
            {
                SubMenuSQL = String.Format(@" SELECT DISTINCT TBL_SubMenuMaster.SubMenuID as MenuID, TBL_SubMenuMaster.SubMenuName  ,SubMenuNavigateURL,TBL_MenuMaster.ModuelName  " +
                            ...
                            "   TBL_MenuMaster ON TBL_SubMenuMaster.MenuID = TBL_MenuMaster.MenuID INNER JOIN    " +
                            "   CTuser ON Roleright.Usertypeid = CTuser.RollId      " +
                            "   WHERE   ModuelName='" + LabName + "' and  (CTuser.USERNAME = '" + Session["username"] + "')  and TBL_SubMenuMaster.MenuID='" + Menuid + "'   and  TBL_SubMenuMaster.Isvisible=1", con);
            }
            ...
            foreach (DataRow SubMainDr in SubMenuDt.Rows)
            {
                ...
                SlideBarHtml += "<li>";
                if (Convert.ToString(SubMainDr["ModuelName"]) == "Pharmacy")
                {
                    SlideBarHtml += "<a href=" + (char)34 + SubMenuUrl + (char)34 + ">" + SubMenuName.ToString() + "</a>";
                }
                if (Convert.ToString(SubMainDr["ModuelName"]) == "Pathology")
                {
                    SlideBarHtml += "<a href=" + (char)34 + SubMenuUrl + (char)34 + ">" + SubMenuName.ToString() + "</a>";
                }
                if (Convert.ToString(SubMainDr["ModuelName"]) == "Radiology")
                {
                    SlideBarHtml += "<a href=" + (char)34 + SubMenuUrl + (char)34 + ">" + SubMenuName.ToString() + "</a>";
                }
                if (Convert.ToString(SubMainDr["ModuelName"]) == "MedicalLab")
                {
                    SlideBarHtml += "<a href=" + (char)34 + SubMenuUrl + (char)34 + ">" + SubMenuName.ToString() + "</a>";
                }
