...
public partial class TestwiseDailyCount : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            try
            {
                if (Convert.ToString(Session["HMS"]) != "Yes")
                {
                    ...
                    {
                        checkexistpageright("TestwiseDailyCount.aspx");
                    }
                }
    ...
    protected void btnreport_Click(object sender, EventArgs e)
    {
        ...
        cmd1.CommandText = "ALTER VIEW [dbo].[VW_TestwiseDailyCount] AS (SELECT  distinct top 99.99 percent       dbo.patmstd.PatRegID, dbo.patmstd.Patregdate, dbo.SubDepartment.subdeptName, dbo.MainTest.Maintestname, dbo.patmstd.PackageCode " +
                   " FROM            dbo.patmstd INNER JOIN "+
                  "  dbo.SubDepartment ON dbo.patmstd.SDCode = dbo.SubDepartment.SDCode INNER JOIN "+
                  "  dbo.MainTest ON dbo.patmstd.MTCode = dbo.MainTest.MTCode " +
                  " WHERE   patmstd.Branchid=" + Convert.ToInt32(Session["Branchid"]) + " and (dbo.patmstd.PackageCode = N'') ";
        ...
        cmd15.CommandText = "ALTER VIEW [dbo].[VW_GetPackageCount] AS (SELECT DISTINCT "+
                 ...
                 "   dbo.patmstd.PackageCode "+
                 "   FROM            dbo.patmstd INNER JOIN "+
                 "   dbo.PackmstD ON dbo.patmstd.PackageCode = dbo.PackmstD.PackageCode " +
        ...
        DataTable dt = new DataTable();
        dt = ObjTB.Get_TestwiseTotalCount(txtdoctor.Text,Convert.ToInt32(RblTestCount.SelectedValue));
        if (dt.Rows.Count > 0)
        {
            ReportDocument RD = new ReportDocument();
            RD.Load(Server.MapPath("~//DiagnosticReport//Rpt_TestwiseDailyCount.rpt"));
            RD.SetDataSource(dt);
            RD.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, true, "TestwiseDailyCount");
        }
