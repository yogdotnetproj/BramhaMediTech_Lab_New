...
public partial class OperatorwiseReport :BasePage
{
        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                if (Convert.ToString(Session["HMS"]) != "Yes")
                {
                    ...
                    {
                        checkexistpageright("OperatorwiseReport.aspx");
                    }
                }
    ...
    void BindGrid()
    {
        try
        {
            ...
            {
                CenterN = Session["CenterCode"].ToString();
            }
            ...
            string modeofpay = RblCashType.SelectedItem.Text;
            GV_CashierReport.DataSource = Cshmst_supp_Bal_C.Get_DPRRepotData_Operator(Todate, fromDate, username, CenterN, Convert.ToInt32(Session["Branchid"]), Convert.ToInt32(Session["DigModule"]), this.labcode_main, modeofpay);
            GV_CashierReport.DataBind();
    ...
    protected void GV_CashierReport_SelectedIndexChanged(object sender, EventArgs e)
    {

    }

    protected void GV_CashierReport_RowEditing(object sender, GridViewEditEventArgs e)
    {
        try
        {
            string username = GV_CashierReport.Rows[e.NewEditIndex].Cells[9].Text;
            ViewState["username"] = username;
            ViewState["tdate"] = GV_CashierReport.Rows[e.NewEditIndex].Cells[0].Text;
    ...
    protected void GV_CashierReport_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowIndex == -1)
            return;
    ...
    protected void GV_CashierReport_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        GV_CashierReport.PageIndex = e.NewPageIndex;
        BindGrid();
    }
    ...
    protected void btnreport1_Click(object sender, EventArgs e)
    {
        ...
        sc22.CommandText = "ALTER VIEW [dbo].[VW_dsbill] AS SELECT        TOP (99.99) PERCENT patmst.CenterCode AS CCode, CasherReport.BillDate AS RecDate, CasherReport.Modeofpay AS Expr2, patmst.Patname AS FirstName, CasherReport.Patienttest AS Test1, CasherReport.Balance, " +
                          ...
                          "  patmst ON CasherReport.Patregid = patmst.PatRegID   where CasherReport.FID=" + ddlFY.SelectedValue + "";//INNER JOIN   Cshmst ON patmst.PID = Cshmst.PID
        if (RblCashType.SelectedItem.Text == "Cash")
        {
            //username = ddlusername.SelectedItem.Text;
            sc22.CommandText += " and  Modeofpay ='Cash'";
        }
        if (RblCashType.SelectedItem.Text == "Card")
        {
            //username = ddlusername.SelectedItem.Text;
            sc22.CommandText += " and Modeofpay ='Credit Card'";
        }
        if (RblCashType.SelectedItem.Text == "Online Gateway")
        {
            //username = ddlusername.SelectedItem.Text;
            sc22.CommandText += " and Modeofpay ='Online Gateway '";
        }
        ...
        Session["rptname"] = Server.MapPath("~/DiagnosticReport/Rpt_dailycashier.rpt");
        Session["reportname"] = "DailyCash";
    ...
    public void BIndFY()
    {
        ddlFY.DataSource = ObjTB.FillFY();
        ddlFY.DataTextField = "FYDate";
