...
public partial class Addshowparameterspace :BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                {
                   // checkexistpageright("Addshowparameterspace.aspx");
                }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                if (SubTestLog_Bal_C.Existsubtest_ST(txtTestName.Text, Request.QueryString["testcode"].ToString(), Request.QueryString["MTCode"].Trim(), Convert.ToInt32(Session["Branchid"])))
                {
                    Label8.Visible = true;
