...
public partial class DrWise_StatisticsReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            try
            {
                ...
                {
                    checkexistpageright("DrWise_StatisticsReport.aspx");
                }
    ...
    protected void btndailytrans_Click(object sender, EventArgs e)
    {
        ...
        string query1 = "ALTER VIEW [dbo].[VW_drLabStatistics_Report_Child] AS (SELECT     distinct   patmst.CenterName, (patmst.PID) AS NumberofExams,patmst.Patregdate,patmst.TestCharges as TestChargesAll, TestCharges.Amount as TestCharges, patmst.Drname, patmst.RefDr " +
        ...
        string query = "ALTER VIEW [dbo].[VW_drLabStatistics_Report] AS (SELECT        patmst.CenterName, COUNT(patmstd.mtcode) AS NumberofExams, " +
                     "   dbo.FUN_GetdrStatistic_count( patmst.CenterName,patmst.RefDr," + DdlMonth.SelectedValue + "," + ddlYear.SelectedValue + ")as NoOfPAtients, " +
                     "   dbo.FUN_GetdrStatistic_Revenue( patmst.CenterName,patmst.RefDr," + DdlMonth.SelectedValue + "," + ddlYear.SelectedValue + ")as NoOfRevenue,patmst.Drname " +

                      "  FROM            patmst INNER JOIN "+
                      "  patmstd ON patmst.PID = patmstd.PID  "+
                      "  where  patmst.IsActive=1 and YEAR(patmstd.Patregdate)=" + ddlYear.SelectedValue + " and Month(patmstd.Patregdate)=" + DdlMonth.SelectedValue + "  " +
                      " GROUP BY patmst.CenterName,patmst.Drname,patmst.RefDr";
        ...
        Session["rptname"] = Server.MapPath("~/DiagnosticReport/Rpt_DrLabStatisticReport.rpt");
        Session["reportname"] = "DrLabStatisticReport";
