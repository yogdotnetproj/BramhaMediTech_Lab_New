...
public partial class ClickSample :BasePage
{
    ...
    public void Bindbanner()
    {
        ...
        if (dtban.Rows.Count > 0)
        {
            ...
            ViewState["DiscountPerValidate"] = Convert.ToSingle(dtban.Rows[0]["DiscountPerValidate"]);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // Label2.Text = "Register No Is :5555";
        ...
        int ss = ObjTB.GetDailyRegNo(1, Convert.ToInt32( Session["financialyear"]));
        Label2.Text = "Your Reg No:"+ Convert.ToString(ss);
        Bindbanner();
        if (!Page.IsPostBack)
        {
           // ViewState["PID"] = "";
            ...
            {
                checkexistpageright("ClickSample.aspx");
            }
            }
            cmbInitial.Focus();
            try
                ...
                if (OBJ_PBC.getallreadyRegister(DateTime.Now.AddMinutes(-2), txtFname.Text.Trim(), int.Parse(txtAge.Text), CountryC))
                {
                    ...
                    try
                    {
                        ...
                        if (PID > 0)
                        {
                            ...
                            PBCL.clientAmount = Convert.ToSingle(ViewState["clientAmount"]);
                            ...
                            if (txtFname.Text.Length > 4)
                            {
                                ...
                                PBCL.P_PUserName = txtFname.Text.Substring(0, 4);
                                PBCL.P_PPassword = txtFname.Text.Substring(0, 4);
                            }
                            else
                            {
                               // PBCL.P_PUserName = txtFname.Text.Trim() + PID;
                               // PBCL.P_PPassword = txtFname.Text.Trim() + PID;
                                PBCL.P_PUserName = txtFname.Text.Trim() ;
                                PBCL.P_PPassword = txtFname.Text.Trim() ;
                            }
                            ...
                            //PBCL.OtherRefDoctor = txt_remark.Text;
                            PBCL.PatientcHistory = txtpatcardno.Text;
                            PBCL.OtherRefDoctor = txtcardexpdate.Text;
                            ...
                            else if (chkWhatapp.Checked == true)
                            {
                                PBCL.P_SocialMedia = 2;
                            }
                            else
                            {
                                PBCL.P_SocialMedia = 3;
                            }

                            PBCL.P_PatientCardNo = txtpatcardno.Text;
                            PBCL.P_PatientCardExpNo = txtcardexpdate.Text;
                            PBCL.P_MainPPID = Convert.ToInt32(ViewState["MPPID"]);
                            int count = 0;
                            if (grdTests.Rows.Count > 0)
                            {
                                    ...
                                    if (grdTests.Rows[p].Cells[3].Text.Trim() == "P")
                                    {
                                        ...
                                        while (dr.Read())
                                        {
                                            try
                                            {
                                                ...
                                                PBCL.ClientTestRate = Convert.ToSingle(grdTests.Rows[p].Cells[5].Text);
                            ...
                            {
                                Report_QRCode(Convert.ToInt32(ViewState["PID"]));
                            }
    ...
    protected void grdTests_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        lblTotTestAmt.Text = "0";
        ViewState["TableTestWise"] = TableTestWise;
        float amtTestTot = 0f, amtcliTestTot = 0f;
        if (grdTests.Rows.Count > 0)
        {
            foreach (GridViewRow gvr in grdTests.Rows)
            {
                ...
                {
                    amtcliTestTot = amtcliTestTot + Convert.ToSingle(gvr.Cells[5].Text.Trim());
                }
            }
        }
        lblTotTestAmt.Text = Math.Round(amtTestTot, 0).ToString();
        ViewState["clientAmount"] = Math.Round(amtcliTestTot, 0).ToString();
    ...
    void AddTestToTable(string MTCode,string TType)
    {
        ...
        if (Session["CenterCode"] != null)
        {
            ...
            try
            {
                spTable = new SpeCh_Bal_C(MTCode, DrMT.ratetypeid, Convert.ToInt32(Session["Branchid"]));//DrMT.ratetypeid
            }
            catch { }
            if (spTable != null)
            {
            ...
            try
            {
                spTable = new SpeCh_Bal_C(MTCode, DrMT.ratetypeid, Convert.ToInt32(Session["Branchid"]),1);//
            }
            catch { }
            if (spTable != null)
            {
                ...
                {
                    row["clientAmount"] = spTable.Amount.ToString();
                }
            }
            else
            {
    ...
    public static string[] FillTests(string prefixText, int count)
    {
        ...
        if (HttpContext.Current.Session["DigModule"].ToString() != null && HttpContext.Current.Session["DigModule"].ToString() != "0")
            sda = new SqlDataAdapter("select PackageCode as MTCode,PackageName as Maintestname, 'P' as Tes from PackMst where (PackageCode like N'%" + prefixText + "%' or PackageName like N'%" + prefixText + "%') and PackageCode in (select PackageCode from PackmstD where SDCode in (select SDCode from SubDepartment where DigModule ='" + Convert.ToInt32(HttpContext.Current.Session["DigModule"]) + "')) UNION " +
                               " select MTCode, Maintestname  ,'T' as Tes from MainTest WHERE ISTestActive=1 and  (MTCode like N'%" + prefixText + "%' or Maintestname like N'%" + prefixText + "%') and SDCode in (select SDCode from SubDepartment where DigModule='" + Convert.ToInt32(HttpContext.Current.Session["DigModule"]) + "') order by Maintestname ", con);
        else
            sda = new SqlDataAdapter("select PackageCode as MTCode, PackageName as Maintestname , 'P' as Tes from PackMst WHERE PackageCode like N'%" + prefixText + "%' or PackageName like N'%" + prefixText + "%' UNION " +
                                " select MTCode, Maintestname , 'T' as Tes from MainTest WHERE ISTestActive=1 and (MTCode like N'%" + prefixText + "%' or Maintestname like N'%" + prefixText + "%') order by Maintestname ", con);
        DataTable dt = new DataTable();
        sda.Fill(dt);
    ...
    protected void ddlCenter_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            int tottestamt = 0, totclitestamt=0;
            foreach (GridViewRow gvr in grdTests.Rows)
            {
                tottestamt += Convert.ToInt32(gvr.Cells[4].Text);
                totclitestamt += Convert.ToInt32(gvr.Cells[5].Text);
            }
            lblTotTestAmt.Text = tottestamt.ToString();
            ViewState["clientAmount"] = totclitestamt.ToString();
    ...
    protected void txttests_TextChanged(object sender, EventArgs e)
    {
        ...
        if (TempsplitTestcode.Length == 1)
        {
            txttests.Text = txttests.Text + "-" + "-" + "T";
              ...
              var mtcode = SplitTestCode[0].ToString().ToUpper().Trim();
              var testname = SplitTestCode[1].ToString().ToUpper().Trim();
              var testType = SplitTestCode[2].ToString().ToUpper().Trim();
              float rate = 0,Clientrate=0;
            ...
            foreach (GridViewRow gvr in grdTests.Rows)
            {
                if (gvr.Cells[4].Text.Trim() != "")
                {
                    ...
                    try
                    {
                        ...
                        clientamt = Convert.ToSingle(gvr.Cells[5].Text.Trim());
                        Clientrate = clientamt;
                    }
                    catch (Exception exc)
                    {
                        if (exc.Message.Equals("Exception aborted."))
                    ...
                    amtclientTestTot = amtclientTestTot + clientamt;
                }
            }
        }
        ...
        //SqlCommand cmd1 = conn1.CreateCommand();// new SqlCommand();
        //cmd1.CommandType = CommandType.StoredProcedure;
        
        //cmd1.CommandText = "procedure name";
        //cmd1.Parameters.AddWithValue("@mtcode", mtcode); // mtcode
        //cmd1.Parameters.AddWithValue("@", ""); // test name
    ...
    public static string[] GetPatientInfo(string prefixText, int count)
    {
       ...
       // SqlConnection con = DataAccess.ConInitForDC();
        ...
        if (HttpContext.Current.Session["DigModule"] != null && HttpContext.Current.Session["DigModule"] != "0")
        {
            sda = new SqlDataAdapter("select PatRegId,MobileNo,Rtrim(Initial.Title)+' '+FirstName+' '+isnull(LastName,'') as Patname from PatientInformation INNER JOIN Initial ON PatientInformation.TitleId = Initial.TitleId where (MobileNo like '" + prefixText + "%')  or (FirstName like N'%" + prefixText + "%')  or (PatRegId like '" + prefixText + "%') ", con);
        }
        else
        {
            sda = new SqlDataAdapter("select PatRegId,MobileNo,Rtrim(Initial.Title)+' '+FirstName+' '+ isnull(LastName,'') as Patname from PatientInformation INNER JOIN Initial ON PatientInformation.TitleId = Initial.TitleId where (MobileNo like '" + prefixText + "%')  or (FirstName like N'%" + prefixText + "%')  or (PatRegId like '" + prefixText + "%') ", con);
        ...
        foreach (DataRow dr in dt.Rows)
        {
           // tests.SetValue(dr["PPID"] + " = " + dr["MobileNo"] + " = " + dr["Patname"], i);
            tests.SetValue(dr["PatRegId"] + " = " + dr["MobileNo"] + " = " + dr["Patname"], i);
            i++;
        }
        ...
        if (txtDisamount.Text != "")
        {
            ...
            if (lbltotalpayment.Text.Trim() != "")
            {
                ...
                if (Rbldisctype.SelectedValue == "Amt")
                {
                    ...
                    if (Convert.ToString(Session["usertype"]) != "Administrator")
                    {
                        float ConvDisPer = ((Convert.ToSingle(txtDisamount.Text) * 100) / Convert.ToSingle(lbltotalpayment.Text));
                        if (Convert.ToSingle(ConvDisPer) > Convert.ToSingle(ViewState["DiscountPerValidate"]) )
                        {
                            Label10.Visible = true;
                            Label10.Text = "Discount(per) not greater than 15%. pls contact Admin";
                            // txtDisamount.Text = "0";
                            // return;
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        if (Convert.ToString(Session["usertype"]) != "Administrator")
                        {
                            if (Convert.ToSingle(txtDisamount.Text) > Convert.ToSingle(ViewState["DiscountPerValidate"]))
                            {
                                Label10.Visible = true;
                                Label10.Text = "Discount(per) not greater than 15%. pls contact Admin";
                                // txtDisamount.Text = "0";
        ...
        try
        {
            if (txtFname.Text != "")
            {
                ...
                if (PatientMNo.Length > 1)
                {
                    ...
                    // PBC.getPermentId();
                    ViewState["MPPID"] = PermanentID;
                    PBC.getPermentId_HM(PermanentID);
                    cmbInitial.SelectedItem.Text = PBC.Initial.Trim();
                    cmbInitial.DataValueField = PBC.Initial.Trim();
                    ...
                    txtBirthdate.Text = Convert.ToString(PBC.DOB1);
                    if (cmbInitial.SelectedItem.Text.Trim() != "Dr." && cmbInitial.SelectedItem.Text.Trim() != "Baby" && cmbInitial.SelectedItem.Text.Trim() != "Baby Of" && cmbInitial.SelectedItem.Text.Trim() != "Doctor" && cmbInitial.SelectedItem.Text.Trim() != "Dr" && cmbInitial.SelectedItem.Text.Trim() != "doctor" && cmbInitial.SelectedItem.Text.Trim() != "dr" && cmbInitial.SelectedItem.Text.Trim() != "baby" && cmbInitial.SelectedItem.Text.Trim() != "baby of" && cmbInitial.SelectedItem.Text.Trim() != "dr.")
                    {
                        ddlsex.Items.Add(PatientinitialLogic_Bal_C.SelectSex(cmbInitial.SelectedItem.Text));
        ...
        if (grdTests.Rows.Count > 0)
        {
            foreach (GridViewRow gvr in grdTests.Rows)
            {
                if (gvr.Cells[4].Text.Trim() != "")
                {
                    ...
                    {
                        amt = Convert.ToSingle(gvr.Cells[4].Text.Trim());
                        Clientamt = Convert.ToSingle(gvr.Cells[4].Text.Trim());
                    }
                    catch (Exception exc)
                    {
                        if (exc.Message.Equals("Exception aborted."))
                    ...
                    amtTestTot = amtTestTot + amt;
                    amtcliTestTot = amtcliTestTot + Clientamt;
                }
            }
        }
        lblTotTestAmt.Text = amtTestTot.ToString();
        ViewState["clientAmount"] = amtcliTestTot.ToString();
        txttests.Text = "";
        lbltotalpayment.Text = amtTestTot.ToString();
        float Bal = Convert.ToSingle(lbltotalpayment.Text) * Convert.ToSingle(Session["Taxper"]) / 100;
        txthstamount.Text = Convert.ToString(Bal);
    ...
    public bool validation()
    {
        ...
        else
        {
                ...
                if (Convert.ToSingle(txtDisamount.Text) > 0)
                {
                    if (Rbldisctype.SelectedValue == "Amt")
                    {
                        ...
                        if (Convert.ToSingle(ConvDisPer) > Convert.ToSingle(ViewState["DiscountPerValidate"]))
                        {
                            ...
                            txtDisamount.Focus();
                            txtDisamount.BorderColor = Color.Red;
         ...
         //if (txtCenter.Text != "COLL DR / HOSP")
         //{
         //    if (txtpaidamount.Text == "")
         //    {
         //        txtpaidamount.Text = "0";
    ...
    protected void BtnReceipt_Click(object sender, EventArgs e)
    {
        ...
        string sql1 = "";
        if(Convert.ToString( ViewState["PID"])!="")
        {
        SqlConnection conn1 = DataAccess.ConInitForDC();
        ...
        sc1.CommandText = "ALTER VIEW [dbo].[VW_cshbill_ClickSample] AS (SELECT        RecM.BillNo, patmst.Patregdate AS RecDate, 'Cash Bill' AS BillType, RecM.AmtPaid AS AmtReceived, RecM.DisAmt AS Discount, patmst.TestCharges AS NetPayment, RecM.AmtPaid, " +
                         "   dbo.FUN_GetReceiveAmt_Balance(1, RecM.PID) AS Balance, RecM.username, RecM.OtherCharges, patmst.PatRegID, patmst.intial, patmst.Patname, patmst.sex, patmst.Age, patmst.Drname, patmst.TelNo, DrMT.DoctorCode, " +
                          ...
                          "  patmst.OtherRefDoctor, RecM.ReceiptNo, RecM.OtherChargeRemark, RecM.PID  , DrMT.address1 as CenterAddress,patmst.QRImage, patmst.QRImageD,Patmst.PatientcHistory ,CONVERT(varchar, patmst.DOB, 105) AS DOB, CONVERT(varchar,  patmst.PatDatOfBirth, 105) AS PatDatOfBirth " +
        ...
        if (RID != "0")
        {
           ...
           // string path = Server.MapPath("/" + Request.ApplicationPath + "/PayReceipt/");
           // string filename1 = Server.MapPath("PayReceipt//" + "$" + Date1 + "$" + ViewState["PID"] + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "PayReceipt" + ".pdf");
           // string filename2 = Server.MapPath("~/&PayReceipt//" + "$" + Date1 + "$" + ViewState["PID"] + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "PayReceipt" + ".pdf");
           // string FilePath = filename2;
           // string vPath = FilePath;
           // string[] Tempsplit;
           // Tempsplit = FilePath.Split('&');
           // string filepath1 = "~/" + "" + Tempsplit[1];

           // System.IO.File.WriteAllText(filename1, "");//filename1
           ...
           // if (dtB.Rows.Count == 0)
           // {
           //     string filepath11 = Server.MapPath("PayReceipt//" + "$" + Date1 + "$" + ViewState["PID"] + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "PayReceipt" + ".pdf");
           //     FileInfo fi = new FileInfo(filepath11);
           //     fi.Delete();
           //     Label10.ForeColor = System.Drawing.Color.Red;
           //     Label10.Text = "Pay Receipt Not generated.. ";
           //     return;
           // }
            //string OrgFile = Server.MapPath("PayReceipt//" + "$" + Date1 + "$" + ViewState["PID"] + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "PayReceipt" + ".pdf");
            //string DupFile = Server.MapPath("PayReceipt//" + "$" + Date2 + "$" + ViewState["PID"] + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "PayReceipt" + ".pdf");
            ...
            //    }
            //}
           // Page.ClientScript.RegisterStartupScript(this.GetType(), "OpenWindow", "window.open('ReportTransfer.aspx?PID=" + ViewState["PID"] + "&TypeB=PayReceipt','_newtab');", true);
    ...
    protected void btnWhatapp_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (Convert.ToString(ViewState["PID"]) != "")
        {
            ...
            //                  "  patmst.OtherRefDoctor, RecM.ReceiptNo, RecM.OtherChargeRemark, RecM.PID , DrMT.address1 as CenterAddress " +
            //                  "  FROM            patmst INNER JOIN " +
            //                  "  DrMT ON patmst.CenterCode = DrMT.DoctorCode AND patmst.Branchid = DrMT.Branchid INNER JOIN " +
            //                  "  MainTest INNER JOIN " +
            //                  "  patmstd ON MainTest.MTCode = patmstd.MTCode AND MainTest.Branchid = patmstd.Branchid ON patmst.PID = patmstd.PID AND patmst.Branchid = patmstd.Branchid INNER JOIN " +
            ...
            sc1.CommandText = "ALTER VIEW [dbo].[VW_cshbill_ClickSample] AS (SELECT        RecM.BillNo, patmst.Patregdate AS RecDate, 'Cash Bill' AS BillType, RecM.AmtPaid AS AmtReceived, RecM.DisAmt AS Discount, patmst.TestCharges AS NetPayment, RecM.AmtPaid, " +
                            "   dbo.FUN_GetReceiveAmt_Balance(1, RecM.PID) AS Balance, RecM.username, RecM.OtherCharges, patmst.PatRegID, patmst.intial, patmst.Patname, patmst.sex, patmst.Age, patmst.Drname, patmst.TelNo, DrMT.DoctorCode, " +
                             "  RecM.CardNo AS DoctorName, MainTest.Maintestname, MainTest.MTCode, patmstd.TestRate, PackMst.PackageName, patmstd.PackageCode, 0 AS DisFlag, patmst.Patusername, patmst.Patpassword, RecM.Comment, " +
                             "  patmst.MDY, patmst.Remark AS PatientRemark, patmst.Pataddress, patmst.PPID, RecM.PaymentType AS UnitCode, RecM.TaxAmount, RecM.TaxPer, dbo.FUN_GetPrintcount(RecM.branchid, RecM.PID) AS PrintCount,  " +
                             "  patmst.OtherRefDoctor, RecM.ReceiptNo, RecM.OtherChargeRemark, RecM.PID  , DrMT.address1 as CenterAddress,patmst.QRImage, patmst.QRImageD " +
                             "  FROM            patmst INNER JOIN " +
                             "  DrMT ON patmst.CenterCode = DrMT.DoctorCode AND patmst.Branchid = DrMT.Branchid INNER JOIN " +
                             "  MainTest INNER JOIN " +
                             "  patmstd ON MainTest.MTCode = patmstd.MTCode AND MainTest.Branchid = patmstd.Branchid ON patmst.PID = patmstd.PID AND patmst.Branchid = patmstd.Branchid INNER JOIN " +
            ...
            if (RID != "0")
            {
                ...
                selectonFormula = ReportParameterClass.SelectionFormula;
                ReportDocument CR = new ReportDocument();
                CR.Load(Server.MapPath("~//DiagnosticReport//Rpt_PayReceipt_clickSample.rpt"));
                SqlConnection con = DataAccess.ConInitForDC();

                SqlDataAdapter sda = null;
                DataTable dt = new DataTable();
                // DataSet1 dst = new DataSet1();
                sda = new SqlDataAdapter("select * from VW_cshbill_ClickSample where PID='" + ViewState["PID"] + "'  ", con);
                ...
                System.IO.File.WriteAllText(filename1, "");
                string exportedpath = "", selectionFormula = "";
                ReportParameterClass.SelectionFormula = "{VW_cshbill_ClickSample.PID}='" + ViewState["PID"] + "' ";
                ReportDocument crReportDocument = null;
                if (CR != null)
                {
                    crReportDocument = (ReportDocument)CR;
    ...
    public void checkexistpageright(string PageName)
    {
        ...
        MenuSQL = String.Format(@"SELECT        Roleright.Rightid, Roleright.Usertypeid, Roleright.FormId, Roleright.FormName, Roleright.Branchid, usr.ROLENAME, " +
              ...
              "  TBL_SubMenuMaster.SubMenuID , CTuser.FrontDiskDisc , CTuser.BillDiskDisc " +
    ...
    public void Report_QRCode(int PID)
    {
        //BarcodeWriter writer = new BarcodeWriter()
        //{
        //    Format = BarcodeFormat.CODE_128,
        ...
        DataTable dtbI = new DataTable();
        // DataSet1 dst = new DataSet1();
        sda = new SqlDataAdapter("SELECT DISTINCT MainTest.TextDesc, patmstd.PID, patmstd.PatRegID FROM  patmstd INNER JOIN MainTest ON patmstd.MTCode = MainTest.MTCode where PID='" + Convert.ToString(PID) + "'   ", con);
        //  sda = new SqlDataAdapter("select distinct PatRegID as BarcodeID,MTCode from Patmstd where PID='" + Convert.ToString(PID) + "'   ", con);
        ...
        if (dtbI.Rows.Count > 0)
        {
            ...
            for (int p = 0; p < dtbI.Rows.Count; p++)
            {
                if (Convert.ToString( dtbI.Rows[p]["TextDesc"]).Trim() == "TextField")
                {
                    CounCode = dtQ.Rows[0]["CountryCode"] + "UrlReport//" + dtQ.Rows[0]["smsString"] + Convert.ToString( dtbI.Rows[p]["PatRegID"]).Trim() + ".pdf";
                    ...
                    sc.Parameters.AddWithValue("@id", Convert.ToString(dtbI.Rows[p]["PatRegID"]).Trim());
                    try
                    {
                        conn.Open();
                        sc.ExecuteNonQuery();
                ...
                {
                    CounCode = dtQ.Rows[0]["CountryCode"] + "UrlReport//" + dtQ.Rows[0]["smsString"] + Convert.ToString(dtbI.Rows[p]["PatRegID"]).Trim() + "D.pdf";
