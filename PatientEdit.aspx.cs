...
public partial class PatientEdit :BasePage
{
    ...
    string PateintName = "", Patregid = "", CenterName = "", labcode_main = "", Barcode = "", Mno = "";
    ...
    void BindGrid()
    {
        try
        {
            ...
            GridEditTest.DataSource = contact.Get_patmst_Edittest(PateintName, Patregid, Convert.ToDateTime(fromdate.Text), Convert.ToDateTime(todate.Text), Convert.ToInt32(Session["Branchid"]), CenterName, labcode_main, "", Barcode, Mno);
    ...
    protected void GridEditTest_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("EditPatientInfo.aspx?PID=" + PID + "&Center=" + ci.CenterCode + "&FType=Edit&PatRegID=" + regno + "&FID=" + FID + "&sdt=" + fromdate.Text + "&edt=" + todate.Text + "&Patname=EditTest");
        }
        catch (Exception exc)
        {
            if (exc.Message.Equals("Exception aborted."))
