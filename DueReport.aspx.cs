...
public partial class DueReport :BasePage
{
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Convert.ToString(Session["HMS"]) != "Yes")
                {
                    ...
                    {
                        checkexistpageright("DueReport.aspx");
                    }
                }
    ...
    void BindGrid()
    {
        ...
        {          
            ...
            //{
                if (ddlCenter.SelectedIndex > 0)
                {
                    GV_DueReport.DataSource = Cshmst_supp_Bal_C.getLoginfoDataForsummaryRemBalance_Center_outs(fromDate, Todate, username, Convert.ToInt32(Session["Branchid"]), Convert.ToInt32(Session["DigModule"]), Convert.ToString(Session["username"]),ddlCenter.SelectedValue);
                }
                else
                {
                    GV_DueReport.DataSource = Cshmst_supp_Bal_C.getLoginfoDataForsummaryRemBalance_Center(fromDate, Todate, username, Convert.ToInt32(Session["Branchid"]), Convert.ToInt32(Session["DigModule"]), Convert.ToString(Session["username"]));
                }
                    GV_DueReport.DataBind();
           // }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        string query = "ALTER VIEW [dbo].[VW_dsdue] AS SELECT DISTINCT top 99.99 percent RTRIM(dbo.patmst.intial) + ' ' + dbo.patmst.Patname AS PatientName, dbo.patmst.PatRegID,dbo.patmst.Phrecdate, dbo.patmst.TestCharges, "+
              "  sum(dbo.RecM.AmtPaid) as AmtPaid, sum(RecM.DisAmt) AS Discount, convert(char(12),RecM.TransDate,105) as RecDate, dbo.patmst.Centercode, dbo.patmst.CenterName, dbo.patmst.DrName, dbo.patmst.PID, "+
              "  dbo.patmst.branchid, dbo.patmst.testname, dbo.RecM.BillNo, dbo.patmst.Patphoneno, dbo.patmst.TelNo   , "+
              "  case when patmst.IsbillBH=0 then  Patmst.TestCharges - (sum(RecM.AmtPaid) + sum(RecM.DisAmt) ) else 0 end as Mainbalance, "+
              "  case when patmst.IsbillBH=1 then Patmst.TestCharges - (sum(RecM.AmtPaid) + sum(RecM.DisAmt) ) else 0 end as BTHbalance ,RecM.username  "+
              "  FROM         dbo.patmst LEFT OUTER JOIN dbo.RecM ON dbo.patmst.branchid = dbo.RecM.branchid AND "+
              "  dbo.patmst.PID = dbo.RecM.PID  "+

              "  group by RTRIM(dbo.patmst.intial) , dbo.patmst.Patname , dbo.patmst.PatRegID,dbo.patmst.Phrecdate, dbo.patmst.TestCharges,patmst.IsActive,dbo.patmst.Branchid "+
              "  , convert(char(12),RecM.TransDate,105),dbo.patmst.CenterCode,dbo.patmst.CenterName,dbo.patmst.Drname,dbo.patmst.PID,dbo.patmst.testname,dbo.RecM.BillNo,dbo.patmst.Patphoneno,dbo.patmst.TelNo "+
              "  ,dbo.patmst.IsbillBH,dbo.RecM.username,(CAST(CAST(YEAR( RecM.TransDate) AS varchar(4)) + '/' + CAST(MONTH( RecM.TransDate) AS varchar(2)) + '/' + CAST(DAY( RecM.TransDate) AS varchar(2)) AS datetime)) having  patmst.isactive=1  and    dbo.patmst.TestCharges - (sum(RecM.AmtPaid) + sum(RecM.DisAmt) )>0  and patmst.branchid=" + Convert.ToInt32(Session["Branchid"]) + "";
        ...
        Session.Add("rptsql", "");
        Session["rptname"] = Server.MapPath("~/DiagnosticReport/Rpt_pendingpayment.rpt");
        Session["reportname"] = "CenterWiseoutstandingDetail";
