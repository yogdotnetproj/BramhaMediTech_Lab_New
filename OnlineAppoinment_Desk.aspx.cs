...
public partial class OnlineAppoinment_Desk : System.Web.UI.Page
{
    ...
    private void FillddlCenter()
    {
        try
        {
            ddlCenter.DataSource = ObjApp.GetAllDr_Centert_Online();
            ...
            if ( Convert.ToString( Session["usertype"]).Trim() == "CollectionCenter")
            {
    ...
    protected void BedDataList_ItemCommand(object source, DataListCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
           // DataList BedDataList = e.Item.NamingContainer as DataList;
            ...
            Response.Redirect("~/OnlineBookAppoinment.aspx?SlotTime=" + SloatNameN[0] + "&Centername=" + Centername + "&BookDate=" + BDate + " &CenterID=" + CenterID + "", false);
