...
public partial class DirectTestReportPrinting : System.Web.UI.Page
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        int PID = PBC.getPID(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));
        if (PID != -1)
        {
            ViewState["PID"] = PID;
        }
    ...
    public void Bindbanner()
    {
        ...
        //            {
        //                ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('PBC to system administrator.');", true);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            try
            {
                ...
                Fill_Labels();
                Patmst_Bal_C PBC = new Patmst_Bal_C(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));
                txtpatientmail.Text = PBC.Email;
                ...
                ArrayList TestCode_AL = (ArrayList)PatSt_new_Bal_C.GetPatst_Authorized_notingroup(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]), Convert.ToInt32(Session["DigModule"]), subdept);
                for (int T_ID = 0; T_ID < TestCode_AL.Count; T_ID++)
                {
                    ...
                    while (ie.MoveNext())
                    {
                        ...
                        Subdepartment_Bal_C SDB = (Subdepartment_Bal_C)ie.Current;
                        TreeNode tn = tvGroupTree.FindNode(SDB.SDCode);
                        if (tn == null)
                        {
                            tn = new TreeNode(SDB.SubdeptName);
                            tn.Checked = true;
                        ...
                        while (IE_Test.MoveNext())
                        {
                            ...
                            TreeNode TN_Test = new TreeNode(MTB_C.Maintestname);
                            TN_Test.Value = MTB_C.MTCode;

                            string PTStatus = (TestCode_AL[T_ID] as PatSt_Bal_C).Patauthicante;


                            bool PStatus = (TestCode_AL[T_ID] as PatSt_Bal_C).Patrepstatus;
                            if (PStatus == true)
                            {
                                TN_Test.Text = MTB_C.Maintestname + "    <span class='btn btn-sm btn-primary'>(Printed" + ")</sapn>";
                               // TN_Test.Checked = false;
                                 TN_Test.Checked = true;
                            }
                            else
                            {
                                TN_Test.Text = MTB_C.Maintestname;
                                TN_Test.Checked = true;
                            }

                            TN_Test.ToolTip = "Main test";
                            TN_Test.NavigateUrl = "#";


                            tn.ChildNodes.Add(TN_Test);
                ...
                ArrayList MT_Code_Disp = (ArrayList)PatSt_new_Bal_C.GetPatst_Authorized_ingroup_new(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]), Convert.ToInt32(Session["DigModule"]), subdept);
                for (int T_ID = 0; T_ID < MT_Code_Disp.Count; T_ID++)
                {

                    string ProfileName = Packagenew_Bal_C.getPNameByCode((MT_Code_Disp[T_ID] as PatSt_Bal_C).PackageCode, Convert.ToInt32(Session["Branchid"])); //12

                    TreeNode tn = tvGroupTree.FindNode((MT_Code_Disp[T_ID] as PatSt_Bal_C).PackageCode);
                    if (tn == null)
                    {
                        tn = new TreeNode(ProfileName);

                        tn.Checked = true;
                        tn.Value = (MT_Code_Disp[T_ID] as PatSt_Bal_C).PackageCode;
                        tn.NavigateUrl = "#";

                        tvGroupTree.Nodes.Add(tn);
                    }

                    ICollection IC_Test = MainTestLog_Bal_C.GetMaintestBy_Code((MT_Code_Disp[T_ID] as PatSt_Bal_C).MTCode, Convert.ToInt32(Session["Branchid"]));
                    IEnumerator IE_Test = IC_Test.GetEnumerator();
                    while (IE_Test.MoveNext())
                    {
                        MainTest_Bal_C MTB_C = IE_Test.Current as MainTest_Bal_C;
                ...
                ArrayList Test_CodeRes = (ArrayList)PatSt_new_Bal_C.getPrintStatusTableByAuthorizedhemogram(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]), "HM");
                for (int T_ID = 0; T_ID < Test_CodeRes.Count; T_ID++)
                {
                    ICollection IC_Tes = SubdepartmentLogic_Bal_C.GET_subdeptName_bycode("HM", Convert.ToInt32(Session["Branchid"]));
                    IEnumerator IE_Tes = IC_Tes.GetEnumerator();

                    while (IE_Tes.MoveNext())
                    {
                        Subdepartment_Bal_C sunDept_C = (Subdepartment_Bal_C)IE_Tes.Current;
                        TreeNode TN_Test = tvGroupTree.FindNode(sunDept_C.SDCode);
                        TN_Test = new TreeNode(sunDept_C.SubdeptName);

                        TN_Test.Checked = true; //subdeptName
                        TN_Test.Value = sunDept_C.SDCode;
                        TN_Test.NavigateUrl = "#";
                        tvGroupGram.Nodes.Add(TN_Test);
                        //testname====================================================================================
                        ...
                        while (ie1.MoveNext())
                        {
                            Subdepartment_Bal_C SubDB = (Subdepartment_Bal_C)ie1.Current;
                            TreeNode TRN_Test = new TreeNode(SubDB.testname);

                            TRN_Test.Value = SubDB.MTCode;
                            TRN_Test.Text = SubDB.testname;

                            TRN_Test.Checked = true;
                            TN_Test.ChildNodes.Add(TRN_Test);
    ...
    public void Fill_Labels()
    {
        ...
        try
        {
            ...
            PB_C = new Patmst_Bal_C(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));

            lblRegNo.Text = Convert.ToString(PB_C.PatRegID);
            lbldate.Text = PB_C.FID;
            ...
            lbldate.Text = Convert.ToString(PB_C.Patregdate);
    ...
    protected void cmdPrint_Click1(object sender, EventArgs e)
    {
            ...
            if (tvGroupTree.Nodes[i].Checked && tvGroupTree.Nodes[i].Depth == 0)
            {
                for (int j = 0; j <= tvGroupTree.Nodes[i].ChildNodes.Count - 1; j++)
                {
                    if (tvGroupTree.Nodes[i].ChildNodes[j].Checked && tvGroupTree.Nodes[i].ChildNodes[j].Depth == 1)
                    {
                        ...
                        TextDesc = MTC.TextDesc;
                        DispTCode = MTC.SDCode;
                        PatSt_Bal_C P_C = new PatSt_Bal_C();
                        dtexistaut = psnew.Check_Authorised_Test(lblRegNo.Text, MTC.MTCode, Convert.ToInt32(Session["Branchid"]), Convert.ToString(Request.QueryString["FID"]));
                        ...
                        P_C.UpdatePrintstatus(Request.QueryString["PatRegID"].ToString(), Request.QueryString["FID"].ToString(), MTC.MTCode, Convert.ToInt32(Session["Branchid"]), Convert.ToString(PrintUserName));
        ...
        if (textflag == true && DescFlag == true)
        {
            ...
            if (Session["usertype"].ToString().Trim() == "CollectionCenter")
            {
                ...
                // urlArr[Arry_Tes_Code.Length] = "CrystalReport_New.aspx?rt=NonDesc&print=p&ut=psc&PatRegID=" + PatRegID + "&FID=" + FID;
                Session.Add("rptsql", "");
                Session["rptname"] = Server.MapPath("~//DiagnosticReport//Pateintreportdescriptive.rpt");
                Session["reportname"] = "DescType";
                Session["RPTFORMAT"] = "pdf";
    ...
    protected void cmdPrint_Click2(object sender, EventArgs e)
    {
        ...
        if (textflag == true && DescFlag == true)
        {
            ...
            urlArr[Arry_Tes_Code.Length] = "Testnormalresult.aspx?rt=NonDesc&RepName=PrintReport//" + "$" + Date1 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Nondescriptive" + ".pdf";

            targetArr[Arry_Tes_Code.Length] = "1";
            featuresArr[Arry_Tes_Code.Length] = "";
            string OrgFileDesc = Server.MapPath("PrintReport//" + "$" + Date1 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Descriptive" + ".pdf");
            string DupFileDesc = Server.MapPath("PrintReport//" + "$" + Date2 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Descriptive" + ".pdf");
    ...
    protected void btnprint_letterhead_Click(object sender, EventArgs e)
    {
        ...
        if (textflag == true && microflag == true)
        {
            ...
            urlArr[Arry_Tes_Code.Length] = "Testnormalresult.aspx?rt=NonDesc&rtt=Lhead&RepName=PrintReport//" + "$" + Date1 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Nondescriptive" + ".pdf";

            targetArr[Arry_Tes_Code.Length] = "1";
            featuresArr[Arry_Tes_Code.Length] = "";
            string OrgFileDesc = Server.MapPath("PrintReport//" + "$" + Date1 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Micro" + ".pdf");
            string DupFileDesc = Server.MapPath("PrintReport//" + "$" + Date2 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Micro" + ".pdf");
        ...
        if (DescFlag == true && microflag == true)
        {
            ...
            urlArr[Arry_Tes_Code.Length] = "Redirect.aspx?rt=NonDesc&RepName=PrintReport//" + "$" + Date1 + "$" + Request.QueryString["PatRegID"].ToString() + "_" + Request.QueryString["FID"].ToString() + "_" + Convert.ToInt32(Session["Branchid"]) + "_" + "Descriptive" + ".pdf";
    ...
    protected void chkispatientsms_CheckedChanged(object sender, EventArgs e)
    {
        if (chkispatientsms.Checked == true)
        {
            ...
            Cshmst.getBalance(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));
            if (Cshmst.Balance > 0)
            {
            ...
            else
            {
                ...
                PatSt.PatRegID = lblRegNo.Text.Trim();
                PatSt.FID = Request.QueryString["FID"];
                string shortform = PatSt.GetShortform(Convert.ToInt32(Session["Branchid"]));
                Patmst_Bal_C PBC = new Patmst_Bal_C(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));
                string pname = PBC.Initial.Trim() + " " + PBC.Patname;
                string mobile = PBC.Phone;
                string email = PBC.Email;
                string msg = "";
    ...
    protected void Chkemailtopatient_CheckedChanged(object sender, EventArgs e)
    {
        ...
        if (Chkemailtopatient.Checked == true)
        {
            ...
            float BAL_Amount = cashmain.Balance;
                    ...
                    if (tvGroupTree.Nodes[i].Checked && tvGroupTree.Nodes[i].Depth == 0)
                    {
                        for (int j = 0; j <= tvGroupTree.Nodes[i].ChildNodes.Count - 1; j++)
                        {
                            if (tvGroupTree.Nodes[i].ChildNodes[j].Checked && tvGroupTree.Nodes[i].ChildNodes[j].Depth == 1)
                            {
                                ...
                                DispTCode = MTC.SDCode;
                                PatSt_Bal_C P_C = new PatSt_Bal_C();
                                P_C.UpdateEmailstatus(Request.QueryString["PatRegID"].ToString(), Request.QueryString["FID"].ToString(), MTC.MTCode, Convert.ToInt32(Session["Branchid"]), Convert.ToString(Session["username"]));
                ...
                if (tvGroupTree.Nodes[i].Checked && tvGroupTree.Nodes[i].Depth == 0)
                {
                    for (int j = 0; j <= tvGroupTree.Nodes[i].ChildNodes.Count - 1; j++)
                    {
                        if (tvGroupTree.Nodes[i].ChildNodes[j].Checked && tvGroupTree.Nodes[i].ChildNodes[j].Depth == 1)
                        {
                            ...
                            DispTCode = MTC.SDCode;
                            PatSt_Bal_C P_C = new PatSt_Bal_C();
                            P_C.UpdateEmailstatus_Doc(Request.QueryString["PatRegID"].ToString(), Request.QueryString["FID"].ToString(), MTC.MTCode, Convert.ToInt32(Session["Branchid"]));
    ...
    protected void chkcentermailLetter_CheckedChanged(object sender, EventArgs e)
    {
        if (chkcentermailLetter.Checked == true)
        {
            ...
            string pname = PBC.Initial.Trim() + " " + PBC.Patname;
            string mobile = PBC.Phone;
            string email = PBC.P_Doctoremail;
            string docemail = DrMT_Bal_C.GetEmaildrNameTable(Request.QueryString["PatRegID"], Request.QueryString["FID"], Convert.ToInt32(Session["Branchid"]));
            createuserlogic_Bal_C aut = new createuserlogic_Bal_C();
            aut.getemail(Session["username"].ToString(), Convert.ToInt32(Session["Branchid"]));
            if (aut.P_email == "")
