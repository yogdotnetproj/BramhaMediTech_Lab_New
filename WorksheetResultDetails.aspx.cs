...
public partial class WorksheetResultDetails : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            try
            {
                if (Convert.ToString(Session["HMS"]) != "Yes")
                {
                    ...
                    {
                        checkexistpageright("WorksheetResultDetails.aspx");
                    }
                }
                bindchecklist();
    ...
    protected void btndailytrans_Click(object sender, EventArgs e)
    {
        ...
        sc21.CommandText = "ALTER VIEW [dbo].[VW_WorklistWithReport] AS SELECT DISTINCT  TOP (99.99) PERCENT " +
                 ...
                 "  , isnull( patmstd.SpecimanNo,0)as SpecimanNo,patmstd.Patauthicante, isnull( ResMst.ResultTemplate,'')as ResultTemplate  FROM            ResMst RIGHT OUTER JOIN "+
                 "   SubDepartment INNER JOIN "+
                 "   MainTest INNER JOIN "+
                 "   patmst INNER JOIN "+
                 "   patmstd ON patmst.PID = patmstd.PID ON MainTest.MTCode = patmstd.MTCode AND MainTest.SDCode = patmstd.SDCode ON SubDepartment.SDCode = MainTest.SDCode ON ResMst.PatRegID = patmstd.PatRegID AND "+
                 "   ResMst.MTCode = patmstd.MTCode AND ResMst.PID = patmstd.PID LEFT OUTER JOIN "+
                 "   SubTest ON MainTest.MTCode = SubTest.MTCode " +
                      " Where 1=1 ";
        if (fromdate.Text != "")
        {
        ...
        Session["rptname"] = Server.MapPath("~/DiagnosticReport/Rpt_DailyWorklistWithReport.rpt");
        Session["reportname"] = "DailyWorklistReport";
